...
| 回答的问题 | 这份待导入数据字段完不完整、OCR 识别对不对、是不是疑似重复？ | 这个已存在的客户，当前处于什么业务状态（`NORMAL`/`PENDING_RENEWAL`/`PENDING_REVIEW`）？ |
...
### 9.4 待实现：`/clients` 页"导入记录"入口（TODO）

现状缺口：`GET /api/clients/import`（9.3 节提到的批次历史列表接口）后端已实现，但前端 `ClientListPage.tsx` 目前只有"批量导入"上传弹窗，上传成功后 `navigate` 到 `/clients/import/:jobId`，**没有任何入口能再回到这个列表**。用户一旦离开进度页（识别耗时较长时很容易发生），就无法找回未处理完的批次，需补齐这一环。

方案（已与用户对齐，暂缓实现）：

- **入口**：`ClientListPage.tsx` 顶部按钮组"批量导入"旁新增"导入记录"按钮。
- **展示形式**：独立弹窗/抽屉内的分页 `Table`，不单独开路由页（量级类似"下载模板"，辅助入口，不占主导航）。
- **列表字段**：创建时间（文件名因 multipart 中文名 multer/busboy latin1 解码乱码问题不展示，与 9.3 节进度页一致）、状态（`QUEUED/PROCESSING/DONE/FAILED`）、总数/成功/待核对/失败四个计数、操作列"查看" → `navigate('/clients/import/:jobId')`。
- **默认排序**：`createdAt desc`。
- **应对"记录过多"**：
  - 接口已支持 `page`/`pageSize` 分页，前端接上 `Table` 的 `pagination` 即可。
  - 接口已支持 `status` 筛选，前端加状态下拉（全部/进行中/已完成）。
  - 对 `reviewCount > 0` 或 `failedCount > 0` 的行加高亮标签（如"有 N 条待核对"），帮用户在历史记录中优先定位"仍需处理"的批次，而非逐条翻找。
  - 若未来数据量进一步增长，可再加时间范围筛选（如"最近 30 天"），暂不预先实现。
