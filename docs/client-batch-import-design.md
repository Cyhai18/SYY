...
### 9.4 `/clients` 页"导入记录"入口（已实现）

现状：`GET /api/clients/import`（9.3 节提到的批次历史列表接口）后端已实现；前端 `ClientListPage.tsx` 顶部按钮组新增"导入记录"按钮，点击以**抽屉（Drawer）**形式展示批次历史列表，支持状态筛选与分页，"查看"跳转 `/clients/import/:jobId`。
